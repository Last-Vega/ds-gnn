...
def convert_data(data:HeteroData, base:torch.Tensor,  meta_path:tuple, flag:bool=False) -> Tuple[csr_matrix, list]:
    """
    HeteroDataの隣接行列からpositive edgeとnegative edgeを作成する
    data: HeteroData
    base: torch.Tensor
    meta_path: tuple
    flag: bool
    ...
    edge_pos = data[meta_path].edge_index
    edge_neg = negative_sampling(edge_pos, num_nodes=base.shape, num_neg_samples=edge_pos.shape[1])
    edge = torch.cat([edge_pos, edge_neg], dim=1)

    edge_index = edge_index_to_list(edge)
    adj = to_dense_adj(edge)[0]
    adj = csr_matrix(adj)

    if flag:
        np.random.shuffle(edge_index)
        edge_index = edge_index[:int(len(edge_index)/10)]
...
def recommendable_items(z_c:torch.Tensor, z_t:torch.Tensor, target_index:int, top_n)->list:
    ...
    # print(f'kumagai gumi xy: {target_xy}')
